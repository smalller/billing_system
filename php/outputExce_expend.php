<?php
    ...
    $objSheet->setTitle("支出账单");    //给当前活动sheet设置名称
    ...
    foreach($all1 as $key=>$val1) {
        $objSheet->setCellValue("A".$j,$num)->setCellValue("B".$j,$val1["expend"])->setCellValue("C".$j,$val1["sort"])->setCellValue("D".$j,$val1["account"])->setCellValue("E".$j,$val1["PS"])->setCellValue("F".$j,$val1["date"]);
        $j++;
        $num++;
    }
    ...
    browser_export("Excel2007","支出账单.xlsx");   //将excel文件输出到浏览器
